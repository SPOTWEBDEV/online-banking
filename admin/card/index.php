                                            ...
                                            <table id="default-ordering" class="table table-hover dataTable"
                                                ...
                                                <thead>
                                                    <tr role="row">
                                                        ...
                                                        <th class="sorting" tabindex="0"
                                                            ...
                                                            style="width: 61.6875px;">Number</th>

                                                            <th class="sorting" tabindex="0"
                                                            aria-controls="default-ordering" rowspan="1" colspan="1"
                                                            aria-label="Amount: activate to sort column ascending"
                                                            style="width: 61.6875px;">Limit</th>

                                                            <th class="sorting" tabindex="0"
                                                            aria-controls="default-ordering" rowspan="1" colspan="1"
                                                            aria-label="Amount: activate to sort column ascending"
                                                            style="width: 61.6875px;">Expiring_date</th>
                                                ...
                                                <tbody>
                                                    ...
                                                    <?php
                                                            ...
                                                            $stament = mysqli_query($connection, "SELECT card.id AS transaction_id, users.id AS user_id, card.status AS transaction_status, card.*, users.* FROM card JOIN users ON users.id = card.user");
                                                            ...
                                                            if (mysqli_num_rows($stament) > 0){
                                                                ...
                                                                while ($data = mysqli_fetch_assoc($stament)){$count++; {?>
                                                                        ...
                                                                        <tr role="row">
                                                                            ...
                                                                            <td><?php  echo   $data['currency'],$data['limit']?></td>
                                                                            <td><?php  echo  $data['date'] ?></td>
                                                                            <?php if ($data['transaction_status'] == 'pending') { echo "<td style='color:blue;'>Pending</td>"; } elseif ($data['transaction_status'] == 'active') { echo "<td style='color:green;'>Confirmed</td>"; } elseif ($data['transaction_status'] == 'dclined') { echo "<td style='color:red;'>Declined</td>"; } elseif ($data['transaction_status'] == 'deactivated') { echo "<td style='color:red;'>deactivated by user</td>"; } ?>
                                                                            <td>
                                                                                <a href="?approve=<?php echo $data['transaction_id'] ?>" class="btn btn-primary">Approve</a>
                                                                                <a href="?decline=<?php echo $data['transaction_id'] ?>" class="btn btn-danger">Decline</a>
                                                                            </td>    
...
<?php
    ...
    if (isset($_GET['decline'])) {
        ...
        $sql = mysqli_query($connection, "UPDATE `card` SET `status` = 'dclined' WHERE `id` = $id");
        ...
        if ($sql) {
            echo "<script>
                Swal.fire({
                    icon: 'success',
                    title: 'Card Declined',
                    text: 'you have successfully declined the card activation.'
                }).then(() => {
            ...
            echo "<script>
                Swal.fire({
                    icon: 'error',
                    title: 'Error',
                    text: 'Unable to update the card status.'
                });
            </script>";
        }
    }
    ...
    if (isset($_GET['approve']) ) {
        ...
        $id = $_GET['approve'];  // sanitize input


        $sql = mysqli_query($connection, "UPDATE `card` SET `status` = 'active' WHERE `id` = $id");
        ...
        if ($sql) {
            ...
            echo "<script>
                Swal.fire({
                    icon: 'success',
                    title: 'Card Approved',
                    text: 'you have successfully approved the card activation.'
                }).then(() => {
                    location.href = 'index.php'; // redirect after alert
                });
            </script>";
