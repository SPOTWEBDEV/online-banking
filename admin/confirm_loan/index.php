                                                ...
                                                <thead>
                                                    <tr role="row">
                                                        ...
                                                        <th class="sorting" tabindex="0"
                                                            ...
                                                            aria-controls="default-ordering" rowspan="1" colspan="1"
                                                            aria-label="Amount: activate to sort column ascending"
                                                            style="width: 61.6875px;">Purpose</th>
...
<?php
    ...
    if (isset($_GET['decline'])) {
        ...
        $sql = mysqli_query($connection, "UPDATE `loan` SET `status` = 'declined' WHERE `id` = $id");
        ...
        if ($sql) {
            ...
            echo "<script>
                Swal.fire({
                    icon: 'error',
                    title: 'Error',
                    text: 'Unable to update the loan status.'
                });
            </script>";
        }
    }
    ...
    if (isset($_GET['approve'], $_GET['user'], $_GET['amount'])) {
        ...
        if ($balance_query && mysqli_num_rows($balance_query) > 0) {
            ...
            $update_user = mysqli_query($connection, "UPDATE users SET loan_balance = $new_balance WHERE id = $user_id");

            if ($update_user) {
                // 4. Update loan status to approved
                $update_loan = mysqli_query($connection, "UPDATE loan SET status = 'approved' WHERE id = $loan_id");
                ...
                if ($update_loan) {
                    echo '<script>
                        Swal.fire({
                            icon: "success",
                            title: "loan Approved",
                            text: "The loan has been approved and balance updated."
                        }).then(() => {
