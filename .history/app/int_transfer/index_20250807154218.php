...
<head>
  ...
  <title>Wire Transaction - <?php echo  $sitename ?> </title>
                            ...
                            <div class="form-group mb-4 mt-4">
                              ...
                              <div class="input-group ">
                                <input type="text" class="form-control" name="acct_name"
                                  placeholder="Beneficiary Account Name" aria-label="notification"
                        ...
                        <div class="row">
                          <div class="col-md-6">
                            <div class="form-group mb-4 mt-4">
                              ...
                              <div class="input-group ">
                                        <select class="form-control" name="bank_name" aria-label="notification" aria-describedby="basic-addon1" required>
                                            <option value="">Select Bank</option>
                                            <?php 
                                                $query = mysqli_query($connection, "SELECT * FROM `bank_list`");
                        ...
                        <div class="row">
                          ...
                          <div class="col-md-6">
                            <div class="form-group mb-4 mt-4">
                              ...
                              <div class="input-group ">
                                <input type="text" class="form-control" name="acct_swift" placeholder="Swift Code"
                                  value="" aria-label="notification" aria-describedby="basic-addon1" required="">
                              </div>
                            </div>
                          </div>
                        </div>
                        ...
                        <div class="row">
                          <div class="col-md-6">
                            <div class="form-group mb-4 mt-4">
                              ...
                              <div class="input-group ">
                                <input type="number" class="form-control" name="acct_routing" value=""
                                  placeholder="Routing Number" aria-label="notification" aria-describedby="basic-addon1"
                                  required="">
                              </div>
                            </div>
                          </div>
                          ...
                          <div class="col-md-6">
                            <div class="form-group mb-4 mt-4">
                              ...
                              <select name="acct_type" id="acct_type" class="form-control selectpicker" data-width="100%" required>
                            ...
                            <div class="form-group mb-4 mt-4">
                              ...
                              <div class="input-group ">
                                <textarea class="form-control mb-4" rows="3" id="textarea-copy"
                                  placeholder="Fund Description" name="acct_remarks" style="resize: none"></textarea>
                              </div>
                            </div>
                          </div>
                        </div>
...
<?php
    ...
    if (isset($_POST['wire_transfer'])) {
        ...
        // Generate secure 6-digit SMS code
        $sms_code      = random_int(100000, 999999);
        $amount        = isset($_POST['amount']) ? trim($_POST['amount']) : '';
        $acct_name     = isset($_POST['acct_name']) ? trim($_POST['acct_name']) : '';
        $bank_name     = isset($_POST['bank_name']) ? trim($_POST['bank_name']) : '';
        $acct_number   = isset($_POST['acct_number']) ? trim($_POST['acct_number']) : '';
        $acct_country  = isset($_POST['acct_country']) ? trim($_POST['acct_country']) : '';
        $acct_swift    = isset($_POST['acct_swift']) ? trim($_POST['acct_swift']) : '';
        $acct_routing  = isset($_POST['acct_routing']) ? trim($_POST['acct_routing']) : '';
        $acct_type     = isset($_POST['acct_type']) ? trim($_POST['acct_type']) : '';
        $acct_remarks  = isset($_POST['acct_remarks']) ? trim($_POST['acct_remarks']) : '';
        $date          = date('Y-m-d');
        $time          = date('H:i:s');

        // Validate account status
        if (isset($account_status) && $account_status === 'suspended') {
            echo "<script>
                Swal.fire({
                    icon: 'error',
                    title: 'Account Suspended',
                    text: 'SWIFT ERROR: Your account is currently suspended. Please contact support.'
                });
            </script>";
            exit;
        }
        ...
        if (floatval($amount) > floatval($balance)) {
            echo "<script>
                Swal.fire({
                    ...
                    text: 'You do not have sufficient balance for this transfer.'
                });
            </script>";
            exit;
        }
        ...
        if (floatval($amount) > floatval($account_limit)) {
            echo "<script>
                Swal.fire({
                    icon: 'warning',
                    title: 'Transfer Limit Exceeded',
                    text: 'The amount exceeds your account transfer limit of {$account_limit}.'
                });
            </script>";
            exit;
        }

        // Prepare and bind statement for security
        $query = "INSERT INTO `transfer` 
                    (`amount`, `type`, `receiver_name`, `description`, `date`, `time`, `status`, 
                    `receiver_account_number`, `receiver_bank`, `swift_code`, `routine_number`, `country`, 
                    `user`, `sender_name`, `receiver_account_type`, `otp_code`) 
                  VALUES (?, 'debit', ?, ?, ?, ?, 'cancelled', ?, ?, ?, ?, ?, ?, ?, ?, ?)";

        if ($stmt = mysqli_prepare($connection, $query)) {
            mysqli_stmt_bind_param(
                $stmt,
                'dsssssssssssss',  // <-- Correct length: 14 chars, matching 14 variables
                $amount,
                $acct_name,
                $acct_remarks,
                $date,
            ...
            if (mysqli_stmt_execute($stmt)) {
                ...
                if (isset($billing_status) && $billing_status == "active") {
                    
                    $_SESSION['otp'] = $sms_code;
                    echo "<script>
                        Swal.fire({
                            icon: 'success',
                            title: 'Transfer Initialized',
                            text: 'Your transaction is pending. Please confirm COT.',
                            confirmButtonText: 'Proceed'
                        }).then(() => {
                            window.location.href = \"cot.php?id={$transfer_id}&table=transfer\";
                        });
                    </script>";
                } else {
                     $_SESSION['otp'] = $sms_code;
                ...
                echo "<script>
                    Swal.fire({
                        ...
                        text: 'An error occurred while initiating the transfer. Please try again later.'
                    });
                </script>";
            }
            ...
            echo "<script>
                Swal.fire({
                    ...
                    text: 'Failed to prepare the transaction query. Please try again later.'
                });
            </script>";
        }
    }
